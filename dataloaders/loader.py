...
def sample_pointcloud(data_path, num_point, pc_attribs, pc_augm, pc_augm_config, scan_name,
    ...
    if random_sample:
        ...
        # # 获取剩余的点索引，排除已采样的有效点
        # remaining_inds = np.setdiff1d(np.arange(N), sampled_valid_point_inds, assume_unique=True)

        # # 确定是否允许重复采样
        # replace = len(remaining_inds) < (num_point - sampled_valid_point_num)

        # # 从剩余点中采样，其实未必仅是无效点
        # sampled_other_point_inds = np.random.choice(
        #     remaining_inds, 
        #     num_point - sampled_valid_point_num,
        #     replace=replace
        # )
        #原始代码
        sampled_valid_point_inds = np.random.choice(valid_point_inds, sampled_valid_point_num, replace=False)
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index, n_way_classes=None):
        ...
        support_ptclouds, support_masks, query_ptclouds, query_labels, support_text_prompts = self.generate_one_episode(sampled_classes)
    ...
    def generate_one_episode(self, sampled_classes):
        ...
        for sampled_class in sampled_classes:
            ...
            #————————————————————Invalid——————————————————————————
            real_type = self.class2type[sampled_class]
            text_prompt = f'A 3D point cloud of a {real_type}.'
            support_text_prompts.append(text_prompt)
            #——————————————————————————————————————————————————————
        ...
        support_text_prompts = np.stack(support_text_prompts, axis=0)


        return support_ptclouds, support_masks, query_ptclouds, query_labels, support_text_prompts
...
def batch_task_collate(batch):
    batch_support_ptclouds, batch_support_masks, batch_query_ptclouds, batch_query_labels, batch_sampled_classes, batch_support_text_prompts = batch[0]

    #把text prompts 加到sampled_classes前面 这样就不会影响测试标签的取值逻辑
    data = [torch.from_numpy(batch_support_ptclouds).transpose(2,3), torch.from_numpy(batch_support_masks),
            torch.from_numpy(batch_query_ptclouds).transpose(1,2), batch_support_text_prompts, torch.from_numpy(batch_query_labels.astype(np.int64))
            ]
...
#     support_ptclouds, support_masks, query_ptclouds, query_labels, sampled_classes, support_text_prompts = data
...
#     data_file.create_dataset('support_text_prompts',data=support_text_prompts, dtype='str')
#     data_file.close()
...
#     support_text_prompts = data_file['support_text_prompts'][:]

#     return support_ptclouds, support_masks, query_ptclouds, query_labels, sampled_classes, support_text_prompts
...
def write_episode(out_filename, data):
    ...
    with h5.File(out_filename, 'w') as data_file:
        ...
        # 特殊处理文本数据
        text_dtype = h5.special_dtype(vlen=str)  # 定义变长字符串类型
        encoded_texts = np.array(support_text_prompts, dtype=text_dtype)
        data_file.create_dataset('support_text_prompts', 
                               data=encoded_texts,
                               dtype=text_dtype)
...
def read_episode(file_name):
    with h5.File(file_name, 'r') as data_file:
        ...
        # 特殊处理文本数据
        text_dataset = data_file['support_text_prompts']
        support_text_prompts = [
